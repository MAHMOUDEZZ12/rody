...
async function PackageImages() {
  const imagePromises = packages.map(pkg => generateSimpleImage({
    prompt: `A beautiful and luxurious lifestyle photograph representing a Rody Wellness spa package experience. The theme should reflect the package name: "${pkg.name}". The image should feature a person in a state of serene relaxation, perhaps wearing a branded robe or enjoying a luxurious home-spa environment with branded towels visible. Use clean, bright lighting with brand colors of soft pink and gold. The aesthetic should be elegant, aspirational, and high-end. Professional photography, high resolution.`,
  }));
