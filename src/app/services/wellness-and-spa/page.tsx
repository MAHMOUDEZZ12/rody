...
async function WellnessPageData() {
  const heroImageUrl = await generateSimpleImage({
    prompt:
      'Sanctuary for the Senses: A tranquil, bright photograph of a woman enjoying a massage on a professional table in her luxurious Dubai home. A female therapist in a branded uniform with a subtle emblem is performing the treatment. The scene has a soft, golden light and features orchids, balanced stones, and branded towels. The background is a soft-focus that evokes peace and wellness, with hints of the brand\'s pink and gold colors. This is a photorealistic, high-resolution photograph.',
  });
  ...
  const imagePromises = wellnessServices.map(service => generateSimpleImage({
    prompt: `A beautiful and luxurious lifestyle photograph showing the experience of a ${service.categories[0]} service from Rody Wellness. The image should capture the essence of "${service.name}", featuring a client in a serene state of relaxation receiving the treatment from a therapist in a branded uniform with a subtle emblem. Keywords for the mood are: ${service.dataAiHint}. Use professional photography style with a clean, elegant background featuring brand colors (soft pink, gold) and bright lighting. High resolution.`
  }));
